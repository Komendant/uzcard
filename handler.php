 ...
 */
class UzcardHandler extends PaySystem\ServiceHandler implements PaySystem\IRefundExtended, PaySystem\IHold
{
	/**
	...
	public function initiatePay(Payment $payment, Request $request = null)
	{
		$params = array(
			...
			'PAYMENT_CURRENCY' => $this->getBusinessValue($payment, "PAYMENT_CURRENCY"),
		);
	...
	{
		return array('BX_HANDLER' => 'UZCARD');
	}
	...
	public function refund(Payment $payment, $refundableSum)
	{
		...
		$shopId = $this->getBusinessValue($payment, 'UZCARD_EPOSID');
		$key = $this->getBusinessValue($payment, 'UZCARD_KEY');
		$requestArrUzcard = array(
				'key' => $key,
				'summa' => number_format($refundableSum),
				'eposid'=> $shopId,
				'uniques'=>$payment->getField('PS_INVOICE_ID')
		);
		$data = json_encode($requestArrUzcard);
		$login = $this->getBusinessValue($payment, 'UZCARD_LOGIN');
		$password = $this->getBusinessValue($payment, 'UZCARD_PASSWORD');
		...
		$result = new PaySystem\ServiceResult();
		if ($content !== false) {
			$contentArrUzcard = json_decode($content);
		}else{
			$error .='От сервера не получен ответ';
		}

		if (isset($contentArrUzcard)){
			
			if (is_null($contentArrUzcard->result)){
				$error .="Ответ платежной системы: ";
				$error .="Код ошибки: ".$contentArrUzcard->error->code.". "."Описание ошибки: ".$contentArrUzcard->error->message;
			}else{
				if($contentArrUzcard->result->success == true){
					$fields = array();
					$fields['PS_STATUS'] = "N";
					$fields['PS_STATUS_DESCRIPTION'] = "Деньги возвращены покупателю";
					$result->setOperationType(PaySystem\ServiceResult::MONEY_LEAVING);
					$result->setPsData($fields);
					Debug::dumpToFile($result,' ', 'refund.log');
				} else {
					$error .="Что-то пошло не так, обратитесь к разработчику";
				}
	...
	protected function getUrlList()
	{
		return array(
			'pay' => array(
				self::ACTIVE_URL => 'http://195.158.28.125:9099/api/PaymentBusiness/paymentsWithOutRegistrationNew'
			),
			'confirm' => array(
				self::ACTIVE_URL => 'http://195.158.18.125:9099/api/payment/CheckTransaction',
			),
			'return' => array(
				self::ACTIVE_URL => 'http://195.158.28.125:9099/api/payment/reversal',
			)
		);
	}
	...
	{
		$sum = $request->get('summa');
		$paymentSum = $this->getBusinessValue($payment, 'PAYMENT_SHOULD_PAY');
		Debug::dumpToFile($sum.'-'.$paymentSum,' ','uzcard_pay_proc.log');
		return roundEx($paymentSum, 0) == roundEx($sum, 0);
	}
...
//		);
	...
	public static function findMyDataRefundablePage(array $paySystemList)
	{
		...
		foreach ($paySystemList as $data)
		{
			...
			{
				$shopId = BusinessValue::get('UZCARD_EPOSID', PaySystem\Service::PAY_SYSTEM_PREFIX.$data['ID'], $personType['ID']);
